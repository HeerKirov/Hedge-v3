...
const { partitionMonthData, partitionData, monthRefs, pmRefs, calendarDate, selectMonth, scrollEvent, openPartition } = useTimelineContext()
...
<template>
    <div :class="$style.timeline">
        <div :class="[$style['left-column'], $style['timeline-list']]" @scroll="scrollEvent">
            <div v-for="pm in partitionData" :key="`${pm.year}-${pm.month}`" :ref="el => pmRefs[`${pm.year}-${pm.month}`] = el">
        ...
        <div :class="[$style['right-column'], $style['timeline-list']]">
            <div v-for="p in partitionMonthData" :key="`${p.year}-${p.month}`" :ref="el => monthRefs[`${p.year}-${p.month}`] = el" >
