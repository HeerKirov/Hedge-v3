...
const menu = useDynamicPopupMenu<CommonIllust>((illust, { alt }) => [
    ...
    {type: "submenu", label: "快捷排序", enabled: selected.value.length > 1, submenu: [
        ...
        {type: "separator"},
        {type: "normal", label: "按文件名顺序重设排序时间", click: i => operators.batchUpdateTimeSeries(i, "SET_ORDER_TIME_BY_FILENAME")},
...
<template>
    ...
    <PaneLayout scope-name="illust" :show-pane="paneState.visible.value">
        ...
        <EmbedPreview/>
        <template #pane>
            <IllustDetailPane tab-scope="find-similar" @close="paneState.visible.value = false"/>
        </template>
    </PaneLayout>
</template>
