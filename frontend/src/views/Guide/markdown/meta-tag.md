...
标签还有“组”类特性。作为父节点的标签可以标记为某一种组，这样它的子标签都会作为组成员。组起到唯一性约束；可选的组特性有：
* **排序组**: 组的成员是有顺序意义的。这意味着这个组的成员在搜索时可以使用范围匹配。
* **覆盖组**: 组的成员是父节点的衍生版本，因此当存在组成员时，对外隐藏父节点，不论父节点是什么类型。
...
* “角色”的父主题可以是版权方、作品、角色任意类型。(角色也是可以有嵌套角色的，在某角色存在一个独立性较强的衍生版本时就可以应用这一点，并且子角色遵循覆盖规则覆盖父角色)
* “版权方”、“作品”的名称，在各自的类型下必须是全局唯一的。
* “角色”的名称则是在**局部主题树**的范围内必须是唯一。
* 此外，还存在“节点”这一特殊的主题类型。节点不能被添加到图像，唯一的作用是组织主题树，充当逻辑节点。
