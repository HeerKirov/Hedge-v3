...
对项目本身的一些文本进行搜索时，有一个更简便的语法。我们使用方括号`[]`将要查询的内容包裹起来：
```
[jp_game]       //=> 查询关键词或描述中包含"jp name"的项
[jp game]       //=> 查询关键词或描述中包含"jp"与"name"的项，且这两个关键词得是在同一个字段内并保持这个先后顺序
[`jp name`]     //=> 查询关键词或描述可以完全匹配"jp name"的项
```
