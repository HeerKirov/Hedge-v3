...
<template>
    <template v-if="!!settingQuery">
        ...
        <p class="secondary-text">查询语句中的中文字符会被识别为有效的符号。会被识别的中文字符包括<code>：～（）【】「」｜，。《》</code>。</p>
        <CheckBox class="mt-2" v-model:value="settingQuery.translateUnderscoreToSpace">转义普通下划线</CheckBox>
        <p class="secondary-text">在受限字符串、关键字等非无限字符串位置的下划线(<code>_</code>)会被转义成空格。</p>
        ...
        <p class="secondary-text">查询结果优化：在对查询语句中的标签进行预查询时，对每个标签元素的单次查询的结果数限制。将此值设置在一个较低的水平，能有效规避输入匹配泛用性过强的查询时，获得的标签结果太多的情况。</p>
        ...
        <p class="secondary-text">查询警告：如果一个元素项中，元素数超出了阈值，就会提出警告。在执行查询时，以OR方式连接的元素数过多，可能拖慢查询效率。</p>
        ...
        <p class="secondary-text">查询警告：如果元素项的总数超出了阈值，就会提出警告。在执行查询时，以AND方式连接的元素数过多，可能严重拖慢查询效率。</p>
    </template>
</template>