...
import { useSettingImportData } from "@/services/api/setting"
...
<template>
    <template v-if="!!settingImport">
        <div class="mt-2">
            ...
            <p class="is-font-size-small secondary-text">导入文件时，自动分析导入项目的来源。</p>
        </div>
        <div class="mt-2">
            <CheckBox v-model:value="settingImport.setTagmeOfTag">自动设定Tagme:标签</CheckBox>
            <p class="is-font-size-small secondary-text">导入文件时，自动将导入项目的Tagme标记为标签、主题和作者。</p>
        </div>
        <div class="mt-2">
            <CheckBox v-model:value="settingImport.setTagmeOfSource">自动设定Tagme:来源</CheckBox>
            <p class="is-font-size-small secondary-text">导入文件时，自动将导入项目的Tagme标记为来源。不过，如果项目的来源可以被分析，则不会设定。</p>
        </div>
        <div class="mt-2">
            <label class="label">排序时间方案</label>
            <Select class="mt-1" :items="timeTypes" v-model:value="settingImport.setOrderTimeBy"/>
            <p class="is-font-size-small secondary-text">使用选定的属性作为导入项目的排序时间。当选定的属性不存在时，自动选择其他属性。</p>
        </div>
        <div class="mt-2">
            ...
            <p class="is-font-size-small secondary-text">从创建时间生成分区时间时，会将0点以后延迟一定时间内的时间点仍然视作前一天。</p>
        </div>
