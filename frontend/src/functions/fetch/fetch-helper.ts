...
export interface FetchHelper<PARAM, MODEL> {
    (param: PARAM): Promise<MODEL | undefined>
}

export interface PostFetchHelper<PARAM> {
    (param: PARAM): Promise<boolean>
}
...
export function useFetchHelper<PARAM, MODEL, E extends BasicException>(options: FetchHelperOptions<PARAM, MODEL, E> | FetchHelperOptions<PARAM, MODEL, E>["request"]): FetchHelper<PARAM, MODEL> {
    ...
    return async (param: PARAM): Promise<MODEL | undefined> => {
        const res = await method(param)
        if (res.ok) {
            afterRequest?.(res.data, param)
            return res.data
        } else if (res.exception) {
            const e = handleErrorInRequest ? handleErrorInRequest(res.exception) : res.exception
...
export function usePostFetchHelper<PARAM, MODEL, E extends BasicException>(options: PostFetchHelperOptions<PARAM, MODEL, E> | PostFetchHelperOptions<PARAM, MODEL, E>["request"]): PostFetchHelper<PARAM> {
    const { httpClient, handleException } = useFetchManager()
    ...
    const call = async (param: PARAM): Promise<boolean> => {
        const res = await method(param)
        if(res.ok) {
            afterRequest?.(res.data, param)
            return true
