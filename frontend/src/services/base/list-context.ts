 ...
 */
interface ListViewContext<T, F extends object> {
...
export function useListViewContext<T, F extends object>(options: ListViewContextOptions<T, F>): ListViewContext<T, F> {
    const queryFilter: Ref<F> = ref(options.defaultFilter ?? {}) as Ref<F>
