...
<template>
    ...
    <template v-if="data?.collection">
        ...
        <ThumbnailImage class="mb-1" :file="data.collection.thumbnailFile" :num-tag-value="data.collection.childrenCount" @click="openRelatedCollection"/>
    </template>
    <template v-if="data?.associates?.length">
        ...
        <div class="has-text-right mb-1">
            <a @click="openAssociate">查看关联组的全部项目<Icon class="ml-1" icon="angle-double-right"/></a>
        </div>
    </template>
