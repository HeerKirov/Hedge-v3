...
* `components`
    ...
    * `forms`: 简单的表单类组件，如输入框、按钮、选择器等。
    * `logicals`: 逻辑组件，如虚拟滚动、懒加载、编辑控制器等。
    * `layouts`: 各类布局框架组件，如侧边栏架构、顶栏架构等。
...
服务层的主要内容有这么几块：
* `modules`: 面向全局安装的、提供全局性的功能服务模块，例如keyboard全局事件服务、document全局控制服务、router全局导航服务等。
