## [Unreleased]
### Fixed
* 修复导入-从文件名解析来源数据功能中，对TAG类型的解析存在的错误。

## [0.18.3] - 2026-03-19
### Fixd
* 修复“推导时根据父标签解决子标签冲突”功能的一处问题。当存在多个父标签，它会要求所有拥有映射标签的父标签的映射标签都要存在于标签列表，而这与常见场景不符。现已将条件更改为只需任一父标签满足前提条件。
* 修复导入页面侧边栏的编辑组件(FormEditKit)在编辑过程中切换选择项，会串项保存的问题。
* 修复在详情页视图的第一项或最后一项附近步进时，若此项是集合，则选择的子项不正确的问题。
* **[SERVER]** 修复构建时的模块uses问题。该问题可能导致本地模式启动失败。
* **[CRX]** 修复Pixiv站点在重命名功能中对个别图像的来源ID识别错误的问题。
* **[CRX]** 修复E-Hentai一键屏蔽按钮只能生效一次的问题。

## [0.18.2] - 2026-01-06
### Added
* **[CRX]** E-Hentai用户屏蔽功能: 添加了评论区的一键屏蔽按钮。
### Fixed
* **[CRX]** 修复书签管理器在创建书签时，相关书签列表表现异常的问题。
* **[CRX]** 修复选项页面中，一部分保存按钮位置错误的问题。
### Optimized
* 调整边缘按钮的圆角弧度以适配macOS 26。
...
### Added
* **[CRX]** 图像列表的扩展工具栏:
  - 在列表页、画廊页，新增了鼠标移入项目时会在右上角出现的弹出按钮，点击即可打开扩展工具栏，快捷执行一些原本需要打开图像页面才能执行的操作。
  - 来源数据收集状态: 直接查看此图像的下载状态和来源数据收集状态，以及也可以直接收集来源数据。在E-Hentai画廊页中，由于本就位于一个来源单位内，仅显示图像下载状态且不包含收集按钮。
  - 相似项查找: 使用封面的缩略图，进行相似项查找，不再需要点进每一个页面。
  - 下载图像: 直接下载原图像。在Pixiv此功能不可用；在E-Hentai中仅在画廊页内可用而在列表页不可用。
* **[CRX]** 来源信息面板的标签映射: 在列表页，来源信息面板可以提取当前列表页中包含的作者类信息，并列出当前映射的元数据标签。
* **[CRX]** 书签管理器-相关书签功能: 新增相关书签显示，按照title自动查找并列出类似的书签。
### Changed
* 导出功能优化: 
  - 通过导出对话框导出文件时，现在可以选择文件命名方式了(原文件名、来源命名、ID)。
  - 通过导出对话框导出文件时，会在Finder/Explorer中打开导出目录。
  - 通过拖曳导出文件时，会按原文件名的方式导出。
* 查询框输入预测调整: 现在不再会在选择标签、元数据标签等时总是添加反引号了。标签名称不包含特殊字符时不会添加任何引号，仅在有必要时添加双引号或反引号。
* 作者/主题新建页面: 添加了编辑名称时的即时检测，会在发现名称已存在时立刻提出警告。
* **[CRX]** 图像详情页的下载工具栏: 已升级为扩展工具栏，除了原先的下载按钮外，也增添了弹出按钮用于打开扩展工具栏。
### Fixed
* 修复作者列表向下滚动一段距离后会出现项距离和偏移量计算错误的问题。
* 修复文件夹目录树编辑状态的一个问题。此前在新建/编辑子节点后，若父节点不在视图内，则会跳转到父节点的位置。
* **[CRX]** 修复下载项管理器无法刷新文件图标的问题。
* **[CRX]** 修复下载项管理器在文件名过长时不会隐藏而是撑破布局的问题。
* **[CRX]** 修复来源信息面板在来源分页名称过长时不会隐藏而是撑破布局的问题。
* **[CRX]** 修复书签管理器中的文件夹选择器的层级位于其他面板下方的问题。
### Optimized
* **[CRX]** 来源信息面板不会再因为URL hash的变化而重新加载。
* **[CRX]** 下载项管理器更换了一部分按钮图标。

## [0.17.0] - 2025-12-16
### Added
* **[CRX]** 新的侧边栏功能:
  - 扩展现已支持SidePanel侧边栏。侧边栏的目标是更方便地在一处集中处理扩展所涉及的工作流程，因此除了包含扩展的核心功能外，还集成了一些浏览器本身具有的功能。
  - 侧边栏可以处理来源信息。和Popu弹出窗口一样，侧边栏也可以随时查看和处理来源信息。
  - 新功能: 格式化书签管理器。当前页面为书签页时，侧边栏会展示经由一定格式化处理的书签页信息。此功能允许在书签页标题上添加信息附加，作为轻量化的数据库使用。
  - 新功能: 下载项管理器。由于工作流程经常涉及，因此集成了浏览器原生的下载管理，并按照使用体验做了一定的优化。
### Changed
* 快捷整理功能优化: 快捷整理功能的参数已被归纳为几种特定的整理选项，以减少使用复杂度。同时，快捷整理的逻辑也已重新实现。
* 克隆图像属性功能优化: 已重新设计为“图像替换”功能。现在具有更易懂的UI和使用方式，减少了不必要的微调选项，以及允许选中多项、一次执行多对替换操作了。
* 相似项查找-加入集合功能优化: 现在使用与图库类似的加入集合对话框，不再使用非常不直观的ID选择菜单。
### Fixed
* 修复“推导时根据父标签解决子标签冲突”功能的一处问题。当父标签在当前来源站点有多个映射标签时，它会要求所有映射标签都必须存在于映射列表，而这与常见场景不符。现已将条件更改为只需任一映射标签。
* 修复主题/作者详情页的别名无法编辑的问题。
* 修复视频播放器进度记忆在特定情况下会失效的问题。该问题是由于视频缓存完成前后使用了不同的文件名，针对此文件名进行记忆造成了记忆丢失的现象。
### Optimized
* 相似项查找、快速查找页面新增了时间分区标识头。每个时间分区的第一项会在左上角标识出它所属的时间分区，以便识别。

## [0.16.2] - 2025-11-25
### Fixed
* 修复来源标签映射编辑器会将空的名称、别名保存下来覆盖原名称的问题。
* 修复一部分集合的Tagme未能正确地从其子项重新生成的问题。
* 修复本地缓存的视频流无法seek的问题。
* 修复页面浏览架构中一个前进记录混乱的问题。当存在前进记录时进行相同页面的Hash路由，前进记录不会被清除。
* 修复一个覆盖组/覆盖主题失效的问题。当父标签的ID在子标签之后时，覆盖效果无法生效。
### Optimized
* 现在主题树会显示角色的子角色，并使用不同的UI样式。

## [0.16.1] - 2025-11-22
### Added
* 新增“根据画师标签映射关系清理Tagme”功能: 在某些站点经常存在画师有来源映射而社团没有的情况，此选项为优化此种情况下的Tagme清理而设计。
### Changed
* 调整“推导时根据父标签解决子标签冲突”功能: 过去此功能的实现方式仅限于一个来源标签有两个确定存在的映射，却不能处理预期之外的其他含义，因此对功能做了一定调整。现在它要求来源映射的角色标签时，某个父标签必须也存在于映射列表中。
### Fixed
* 修复节点类型的主题详情页无法显示关联的项目的问题。已经修改了关联项目的查询方式，改为使用其子标签查询。
* 修复当主题被修改为节点类型后仍会在项目的标签列表看到的问题。
### Optimized
* 主题、作者详情页：当没有关联的画集时，不再显示画集相关内容。
* 调整了部分站点的标签类型的顺序。
* 调整了部分排序时间菜单项的顺序。
...
### Changed
* 主题组织结构改进:
  - 主题现在以局部主题树的形式组织呈现。以每一个根标签为根，在它的子主题范围内维持整棵树的有序，就像标签树那样。
  - 新增了“节点”类型。节点类型类似标签的虚拟地址段，仅用于组织作用，不能被添加到项目，不能在主题列表直接看到，也不会被索引。
  - 在主题列表与主题搜索选取器等位置，为查询HQL新增了parent筛选，可以使用根主题名称直接筛选子主题。
  - 在主题列表调整了父主题过滤器的生效方式，现在它将按照根主题查询。
  - 为角色和子角色新增了“覆盖”特性。若项目拥有子角色标签，将隐藏它拥有的父角色标签。
* 标签组织结构调整:
  - “组”特性进行了简化，移除了强制唯一组特性，并将排序特性合并至组。现在组已重命名为排序组。
  - 新增了“覆盖组”特性。当父标签被声明为覆盖组时，若项目具有该父标签的直接子标签，将隐藏父标签。
* 主题、作者详情页更新: 重新设计交互逻辑，不再需要进入编辑模式，而是与标签类似的对每种属性单独编辑。此外，也调整了右上角弹出菜单与下方示例的内容。
### Fixed
* **[CRX]** Kemono: 修复被重命名带有前缀的附件下载时不会自动收集来源数据的问题。
* 修复页面浏览架构中一个前进记录混乱的问题。当前进记录存在Hash记录时，跳转到其他页面再回退，之前的Hash记录不会被清除。
* 修复Electron版本升级带来的右键菜单偶尔会点击失效的问题。
* 修复Electron版本升级带来的本地缓存视频流无法加载的问题。
### Optimized
* 集合详情页面-左侧集合面板的ID组件不再能切换至文件名显示。

## [0.15.0] - 2025-11-08
### Added 
* 鼠标手势: 添加了鼠标手势模块，按住鼠标右键划出方向来控制导航。
* 文件名外显: 可以查看图像的文件名，以及利用文件名做一些额外的操作。
  - 在图库侧边栏点击项目的ID的图标，可以将显示模式切换至文件名显示；
  - 多选模式下，新增了“根据文件名重新排序”的排序时间菜单项；
  - 在导入页面侧边栏，可以双击文件名以编辑刚刚导入的项目的文件名。
* 视频播放器进度记忆: 视频播放器会记住每个视频的播放进度，并自动恢复到上次播放的时间点。此记忆仅在会话内有效。
### Changed
* 作者列表页面的UI已重新设计。现已根据宽度自动调整列数，并且调整了右键菜单的选项。
* 主题列表页面的UI进行了微调，并且调整了右键菜单的选项。
* 内置站点已统一支持新的文件名识别模板，以配合扩展的改动。
* 视频组件快捷键调整: 现在，在普通页面打开的视频组件，不再响应`Space`快捷键，不干扰关闭预览行为。
* **[CRX]** 为“文件下载重命名”机制引入了新的模板。对于E-Hentai这类图像具有有意义的站点，使用新模板在保留原文件名的同时附加来源信息。这个机制对Kemono站点的视频附件扩展功能同样有效。
### Fixed
* **[SERVER]** 修复了在并发优化后暴露的一部分因未上锁的写入导致的并发写错误。
* **[CLIENT]** 修复macOS 26的兼容性问题。
* 修复了因设置图像的来源，使其Tagme发生变化时，不会导出到集合的问题。
* 修复了主题列表的搜索条件不会被正确计入导航历史的问题。

## [0.14.3] - 2025-10-20
### Added
* **[CRX]** Kemono站点扩展功能: Attachment附件区域的视频类型的文件下载支持直接重命名为标准格式。
### Changed
* 视频组件快捷键调整: 现在，在普通页面打开的视频组件，其前进/后退快捷键为`Meta+Arrow`，不干扰一般方向键；在StackedView打开的视频组件，其前进/后退快捷键仍然为`Arrow`，可以通过`Alt+Arrow`使用一般方向键，或直接使用上下键。
* 导航快捷键调整: 浏览器进行前进/后退的快捷键调整为了`Meta+[`/`Meta+]`，不再与视频组件或其他方向键导航重合。
### Fixed
* **[CRX]** Kemono站点: 针对初始化脚本加载失败的情况进行优化，减少故障频率。
### Optimized
* 视频组件添加了一个底部进度条，在控制组件未浮现时用来显示进度。
* 在编辑集合的子项的标签时，现在可以同多选操作时一样，使用子项的来源数据进行来源推导了。
### Performance
* **[SERVER]** 并发优化: 为后端更换连接池，以优化多个较重查询任务并发执行的串行问题。
...
* 修复了在详情页视图进行首尾循环时，若目标项是集合，则初始的集合子项不正确的问题。该问题是由于计算变更方向时是根据索引变化简单计算获得，没有考虑首尾循环时的特例。
...
* **[CRX]** 相似项查找: 修复浏览对话框的侧边栏的时间组件显示时间分区与排序时间的关系不正确的问题。
* **[CRX]** 附件下载重命名: 修复在Fanbox站点无法生效的问题。

## [0.14.1] - 2025-08-17
### Fixed
* 修复了从一个时间分区详情页直接跳转至另一个时间分区详情页(例如通过侧边栏点击切换)时，查询条件没有传递的问题。
* **[CLIENT]** 修复了本地模式下调用后台服务错误的问题。
### Optimized
* 作者/主题的新建页面不再记忆上一次使用的类型，而是默认使用画师/角色类型。

## [0.14.0] - 2025-07-30
### Changed
* 页面浏览架构升级:
  - 图库中，查询条件的更改现在也被纳入导航历史了。在通过导航在查询条件间跳转时，仅更改当前页面的查询条件，不会对页面的其他状态产生影响。
  - 移除了图库、时间分区、时间分区详情页面在同一个选项卡下的查询条件共享，取而代之的是跨路由时的查询条件传递。也就是说，现在仅会在从一个路由跳到另一个路由时传递它，而在中途离开去往别的页面后此查询条件会自然丢失。这样的机制更加符合在查询条件也被纳入导航历史时的表现预期。
  - 历史记录跳转: 可以右键回退和前进按钮，以浏览和跳转导航历史记录了。
  - 关闭的标签页: 可以通过“新建标签页”按钮的右键菜单快速重新打开之前关闭的标签页了，以及新增了对应的快捷键直接打开上一个关闭的标签页。
  - 选项卡模块: 可以直接拖拽选项卡排序了。
  - 选项卡标题: 图库、时间分区、画集、作者、主题、来源数据等页面的选项卡会在标题显示当前的查询条件，以便更好地从标题区分它们。
### Fixed
* **[CRX]** 添加对Kemono新域名的支持。
...
* 对首页做了优化。
  ...
  - 首页的主题、作者、最近导入、历史时间线等位置的图像列表调整为项目列表，也就是包含了集合/图像。这使得这些地方不至于被单个集合的图像占领。
* 主题、作者详情页: 示例列表也调整为项目列表。这使得这些地方不至于被单个集合的图像占领。
