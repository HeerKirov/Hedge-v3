...
class IllustManager(private val data: DataRepository,
     ...
     * TODO 在更改meta exporter机制后，移除score参数
     */
    ...
    private fun cloneProps(fromIllust: Illust, toIllust: Illust, props: ImagePropsCloneForm.Props, merge: Boolean) {
        ...
        if(parentChanged) {
            ...
            if(newParent != null) processCollectionChildrenAdded(newParent.id, toIllust, now, exportScore = true)
            if(toIllust.parentId != null) processCollectionChildrenRemoved(toIllust.parentId, listOf(toIllust), now, exportScore = true)
        }else{
